...
class ModoProcesamiento {
    ...
    public function cambiarEstado() {
        if ($this->id && $this->estado) {
            $consulta = "UPDATE psa_modo SET estado = ? WHERE id = ?";
    ...
    public function crear() {
        if ($this->nombre) {
            $consulta = "INSERT INTO psa_modo OUTPUT INSERTED.id VALUES (?, 'Activo')";
    ...
    public function modificar() {
        if ($this->id && $this->nombre) {
            $consulta = "UPDATE psa_modo SET nombre=? WHERE id=?";
    ...
    public function obtener() {
        if ($this->id) {
            ...
            $this->mensaje = "No se obtuvo la información del modo de procesamiento";
            return 1;
        }
        $this->mensaje = "No se pudo hacer referencia al modo de procesamiento";
        return 0;
    }

    private function registrarActividad($operacion, $id) {
        $creacion = Log::guardarActividad("psa_modo", $operacion, $id);
