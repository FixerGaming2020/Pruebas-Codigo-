...
class Personal {
    ...
    public function cambiarEstado() {
        if ($this->id && $this->estado) {
            $consulta = "UPDATE per_personal SET estado = ? WHERE id = ?";
    ...
    public function crear() {
        if ($this->sigla && $this->nombre) {
            $consulta = "INSERT INTO per_personal OUTPUT INSERTED.id VALUES (?, ?, ?, ?, 'Activo')";
    ...
    public function modificar() {
        if ($this->id && $this->sigla && $this->nombre) {
            $consulta = "UPDATE per_personal SET sigla = ?, nombre = ?, departamento = ?, rti = ? WHERE id = ?";
    ...
    public function obtener() {
        if ($this->id) {
            $consulta = "SELECT * FROM per_personal WHERE id = ?";
    ...
    private function registrarActividad($operacion, $id) {
        $creacion = Log::guardarActividad("per_personal", $operacion, $id);
