...
$(document).ready(function () {
    ...
    var formOriginal = $("#formModificarProveedor").serialize();

    /* DETECTA LOS CAMBIOS EN LOS CAMPOS DEL FORMULARIO PARA ACTIVAR/DESACTIVAR EL BOTON DE GUARDADO */

    $("#formModificarProveedor").change(function () {
        var formModificado = $("#formModificarProveedor").serialize();
        var habilitar = (formOriginal !== formModificado) ? false : true;
        $("#btnModificarProveedor").prop("disabled", habilitar);
    });

    /* ENVIA EL FORMULARIO PARA REALIZAR LA MODIFICACION */

    $('#formModificarProveedor').submit(function (event) {
        event.preventDefault();
        $.ajax({
            type: "POST",
            dataType: 'json',
            url: "./PModificarProveedor.php",
            data: $("#formModificarProveedor").serialize(),
            success: function (data) {
                $('#seccionResultado').html(data[0]['resultado']);
                if (data[0]['exito'] === true) {
                    $('#formModificarProveedor').find('input, textarea, select').prop('disabled', true);
