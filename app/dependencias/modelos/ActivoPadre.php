...
class ActivoPadre {
    ...
    public function crear() {
        if ($this->categoria && $this->sigla && $this->nombre && $this->hijos) {
            ...
            $datos = array(&$this->sigla, &$this->nombre, &$this->categoria);
    ...
    public function modificar() {
        if ($this->id && $this->categoria && $this->sigla && $this->nombre && $this->hijos) {
            ...
            $this->mensaje = utf8_encode($this->nombre) . ": " . SQLServer::instancia()->getMensaje();
            if ($modificacion == 2) {
                $sborrar = Dependencia::borrar($this->id);
                $this->mensaje = ($sborrar == 2) ? $this->mensaje : Dependencia::getMensaje();
                $screar = Dependencia::crear($this->id, $this->hijos);
                $this->mensaje = ($screar == 2) ? $this->mensaje : Dependencia::getMensaje();
    ...
    private function obtenerActivosHijos() {
        $hijos = ActivosHijos::listarActivosPadre($this->id);
        if (gettype($hijos) == "resource") {
            $this->hijos = array();
            while ($hijo = sqlsrv_fetch_array($hijos, SQLSRV_FETCH_ASSOC)) {
                $this->hijos[] = array("id" => $hijo['id'], "nombre" => utf8_encode($hijo['nombre']));
            }
            return 2;
        }
        $this->mensaje = "Hijos del activo: " . ActivosHijos::getMensaje();
        return 1;
    }
